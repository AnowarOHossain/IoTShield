...
class AnomalyDetector:
    ...
    def get_sensor_thresholds(self, sensor_type):
        ...
        thresholds = {
            'TEMPERATURE': {'min': -10, 'max': 50, 'critical': 45},
            'HUMIDITY': {'min': 0, 'max': 100, 'critical': 90},
            'GAS': {'min': 0, 'max': 1, 'critical': 0.6},
            'FLAME': {'min': 0, 'max': 1, 'critical': 0.8},
            'MOTION': {'min': 0, 'max': 1, 'critical': 1},
            'LIGHT': {'min': 0, 'max': 1000, 'critical': 900},
        }
        
        return thresholds.get(sensor_type, {'min': 0, 'max': 100, 'critical': 80})
    ...
    def check_threshold_violation(self, sensor_data):
        ...
        if sensor_data.value < thresholds['min']:
            return True, 'LOW', f"Value below minimum threshold ({thresholds['min']})"
        elif sensor_data.value > thresholds['critical']:
