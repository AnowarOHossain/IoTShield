...
\noindent Consequently, the fundamental research gap lies in the absence of a holistic, decentralized smart home framework that unifies these disparate elements. There is a distinct lack of systems that successfully combine application-layer cryptographic payload encryption, privacy-preserving data obfuscation, and edge-deployed Generative AI to provide secure ~\cite{ref7}, real-time, and explainable anomaly detection within a lightweight MQTT ecosystem.
...
\begin{itemize}
    \item \textbf{To design a lightweight, edge-centric IoT architecture:} Develop a responsive smart home network utilizing ESP32 microcontrollers, a Raspberry Pi edge gateway, and the MQTT protocol to ensure low-latency communication without heavy reliance on external cloud infrastructure ~\cite{ref8}.
...
\noindent By aggregating these diverse metrics onto a single ESP32 node, the hardware architecture provides the localized AI model with a rich, multivariate dataset ~\cite{ref31}. This allows the system to cross-reference multiple data points (e.g., detecting a simultaneous spike in temperature, gas, and a positive flame reading) to accurately diagnose complex hazards. Table 3.1 provides a detailed technical specification of the hardware components utilized in the physical edge node deployment.
...
\noindent The final tier of the IoTShield system is the intelligent inference framework, which transforms raw environmental telemetry into actionable, human-readable insights. This framework is designed to operate entirely at the edge, utilizing a two-stage detection pipeline: a high-speed threshold-based screening layer and a context-aware Generative AI reasoning layer ~\cite{ref34}. By hosting the inference engine locally, the system ensures that sensitive domestic data is never exposed to third-party cloud providers, fulfilling the primary privacy objectives of the research.
...
\noindent Finally, to ensure the integrity of the data pipeline, the backend was monitored to validate the handshake between the Django framework and the Ollama inference engine  ~\cite{ref44}. As shown in Figure 4.8, the backend processing logs confirm that sensor payloads are correctly routed through the validation logic before being permanently stored, ensuring that only valid, decrypted data reaches the persistent storage layer.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|p{4.5cm}|c|c|p{5cm}|}
        ...
        \hline
        \textbf{Total End-to-End Latency} & \textbf{420} & \textbf{670} & Total traversal time from hardware to user visualization. \\
        \hline
    \end{tabular}
    }
...
\noindent The results demonstrate that the vast majority of alerts (approximately 51.7\%) were low-severity climate deviations, which the system efficiently processed without overwhelming the user with critical alarms. However, the system successfully isolated 68 critical events—primarily driven by direct flame detection or a simultaneous spike in combustible gas and temperature ~\cite{ref49}. In these instances, the threshold engine immediately bypassed standard logging, prioritizing the AI inference payload.

\vspace{0.3cm}

\noindent Figure 5.1 displays the active dashboard interface utilized during the evaluation. The screenshot provides visual confirmation of the system successfully capturing, categorizing, and displaying the 1,568 anomalies, alongside the contextual safety suggestions generated by the Llama 3.2 model. This confirms the system's ability to seamlessly integrate high-speed threshold detection with sophisticated, human-readable AI insights.

\vspace{0.5cm}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{Images/Alerts Page.png}
    \caption{The IoTShield user dashboard actively displaying the alert log, successfully capturing and categorizing the 1,568 environmental anomalies detected during the evaluation phase.}
    \label{fig:alerts_page_results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Images/Gmail-SMTP-Alerts.png}
    \caption{Verification of the automated SMTP delivery system, demonstrating the successful transmission of HIGH and CRITICAL AI-generated safety alerts to the user's email inbox.}
    \label{fig:smtp_email_alerts}
\end{figure}
...
\noindent A fundamental challenge in designing privacy-preserving IoT systems is managing the inherent conflict between data confidentiality (privacy) and system accuracy (utility). In the IoTShield framework, this balance is dictated by the localized Differential Privacy (DP) layer executing on the ESP32. By injecting Gaussian noise into the raw telemetry, the system successfully masks granular behavioral routines ~\cite{ref52}. However, excessive noise could theoretically cause the system to miss genuine hazards (false negatives) or generate spurious warnings (false positives).

\vspace{0.3cm}

\noindent To evaluate this trade-off, the system's performance was analyzed under the configured privacy budget of $\epsilon = 0.5$. The results indicated that the application of noise effectively flattened micro-variations in the environmental data. For example, standard fluctuations in room temperature caused by human presence were statistically hidden within the noise envelope, preventing an adversary from deducing room occupancy based on thermal signatures alone. 

\vspace{0.3cm}

\noindent Despite this obfuscation, the system maintained a high utility rate for anomaly detection. Because the threshold engine was calibrated to identify macroscopic environmental deviations—such as the sharp, sustained spike in MQ-2 readings during a gas leak or the binary trigger of the IR Flame sensor—the injected noise did not interfere with critical hazard identification. The 1,568 successfully categorized alerts logged during the testing phase demonstrate that the chosen $\epsilon$ value provides an optimal equilibrium: it guarantees structural privacy for daily routines while preserving the absolute fidelity required for life-safety monitoring ~\cite{ref53}.
...
\noindent First, against \textbf{Eavesdropping and Man-in-the-Middle (MitM) Attacks}, the system leverages RSA-2048 encryption with OAEP padding ~\cite{ref54}. Even though the Mosquitto broker operates on the standard, unencrypted port 1883 to reduce network latency, the payloads themselves are completely encrypted before leaving the ESP32. If a malicious actor were to intercept the Wi-Fi traffic or compromise the local MQTT broker, they would only acquire OAEP-padded ciphertext. The randomized nature of OAEP ensures that identical sensor readings produce entirely different ciphertexts, rendering statistical replay attacks and known-plaintext attacks computationally infeasible.

\vspace{0.3cm}

\noindent Second, against \textbf{Cloud Data Leaks and Third-Party Profiling}, the system enforces strict data sovereignty. Traditional smart home architectures routinely transmit unencrypted telemetry to third-party cloud servers for AI processing, exposing users to corporate data monetization and server-side breaches. By deploying the Llama 3.2 inference engine locally via Ollama, the IoTShield framework establishes a "zero-trust" boundary ~\cite{ref55}. The raw, decrypted data only exists within the volatile memory of the Django backend and the local SQLite database. No external APIs are queried, ensuring that sensitive domestic data never traverses the public internet.

\vspace{0.3cm}

\noindent Finally, against \textbf{Behavioral Inference Attacks}, the aforementioned Differential Privacy layer acts as the final safeguard. In the hypothetical event that the central edge server is physically stolen or the SQLite database is compromised, the historical telemetry recovered by the attacker would be inherently noisy. The Gaussian perturbation guarantees that exact human routines—such as cooking schedules or sleep patterns—cannot be reliably reverse-engineered, thereby fulfilling the project's core mandate of privacy-by-design.

\vspace{0.5cm}

\section{Comparative Discussion}

\noindent To fully contextualize the contributions of the IoTShield framework, its performance and architectural paradigms must be evaluated against standard, commercially available smart home baselines. Traditional IoT ecosystems overwhelmingly rely on cloud-centric architectures, prioritizing rapid deployment, low hardware costs, and minimal computational overhead at the edge. However, this convenience fundamentally compromises data sovereignty, as sensitive domestic telemetry is continuously transmitted, processed, and stored in plaintext on third-party servers. 

\vspace{0.3cm}

\noindent Table 5.3 presents a comparative analysis between the proposed IoTShield framework and two standard baseline models: a traditional Cloud-Based IoT system and a standard Local Edge system lacking cryptographic payloads.

\vspace{0.5cm}

\begin{table}[H]
    \centering
    \caption{Performance Comparison of IoTShield vs. Baseline IoT Systems}
    \vspace{0.2cm}
    \renewcommand{\arraystretch}{1.5}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{|p{3.5cm}|p{4cm}|p{4cm}|c|p{3.5cm}|}
        \hline
        \textbf{System} & \textbf{Privacy Mechanism} & \textbf{Anomaly Detection} & \textbf{Average Latency} & \textbf{End-to-End Encryption} \\
        \hline
        Traditional Cloud IoT & None (TLS in transit only) & Cloud-based API \& Rules & $\sim$200 ms & No (Server plaintext) \\
        \hline
        Standard Edge IoT & None & Local Thresholds & $\sim$150 ms & No (Network plaintext) \\
        \hline
        \textbf{IoTShield (Proposed)} & \textbf{DP (Gaussian) + RSA-2048} & \textbf{Local AI (Llama 3.2)} & \textbf{$\sim$420 ms} & \textbf{Yes (Payload level)} \\
        \hline
    \end{tabular}
    }
...
\noindent The development of the IoTShield framework establishes a robust foundation for privacy-preserving ~\cite{ref60}, edge-based domestic monitoring. However, as the IoT landscape rapidly evolves, several avenues for future research and optimization remain to scale this prototype into a highly deployable consumer solution. Based on the technical constraints identified during this study, the following research directions are proposed:

\vspace{0.3cm}

\begin{itemize}
    \item \textbf{Transition to Elliptic Curve Cryptography (ECC):} To address the significant computational overhead introduced by RSA-2048 encryption, future iterations of the edge node firmware should explore the implementation of Elliptic Curve Cryptography. ECC can provide an equivalent level of cryptographic security with significantly smaller key sizes (e.g., a 256-bit ECC key is broadly comparable to a 3072-bit RSA key), dramatically reducing modular exponentiation times and freeing up CPU cycles on the ESP32 ~\cite{ref61}.
    
    \vspace{0.2cm}
    
    \item \textbf{Integration of Hardware AI Accelerators:} To lower the barrier to entry for the central edge server, future research should investigate offloading the Large Language Model inference to dedicated local neural processing units (NPUs) or edge AI accelerators, such as the Google Coral TPU or NVIDIA Jetson platforms. Furthermore, utilizing highly quantized versions of Llama 3.2 (e.g., 4-bit quantization) could further reduce RAM requirements without sacrificing contextual reasoning accuracy.
    
    \vspace{0.2cm}
    
    \item \textbf{Decentralized Mesh Networking:} To eliminate the single point of failure inherent to standard 2.4 GHz residential Wi-Fi, the framework could be adapted to operate over low-power, decentralized mesh protocols such as Thread, Zigbee, or Bluetooth Mesh. Incorporating an out-of-band communication fallback, such as a localized LoRaWAN module, would ensure that critical life-safety alerts are transmitted even during targeted network jamming or localized power outages.
    
    \vspace{0.2cm}
    
    \item \textbf{Automated Public Key Infrastructure (PKI):} For large-scale commercial deployment, managing static cryptographic keys on individual nodes is impractical. Future work should design an automated, lightweight certificate enrollment protocol specifically tailored for resource-constrained IoT devices. Exploring blockchain-based identity management or secure multiparty computation (SMPC) could provide a trustless, scalable solution for dynamic key rotation.
    
    \vspace{0.2cm}
    
    \item \textbf{Federated Learning Integration:} Finally, the system's analytical capabilities could be enhanced by implementing a Federated Learning architecture. By allowing multiple independent IoTShield networks to train local anomaly detection models and share only the encrypted weight updates—rather than raw telemetry—the overall AI engine could continuously learn from diverse environmental hazards across thousands of households while mathematically guaranteeing zero exposure of private domestic data.
\end{itemize}
...
\bibitem{ref1}
T. Alam, M. Rokonuzzaman, S. Sarker, A. F. M. Z. Abadin, T. Debnath, and M. I. Hossain, ``Internet of Things-based Home Automation with Network Mapper and MQTT Protocol,'' \textit{Computers and Electrical Engineering}, vol. 120, p. 109807, 2024. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S0045790624007341}
\bibitem{ref2}
P. Cosper, ``Generative AI Techniques for Anomaly Detection in IoT Devices,'' \textit{ResearchGate}, 2025. [Online]. Available: \url{https://www.researchgate.net/publication/395801862_GENERATIVE_AI_TECHNIQUES_FOR_ANOMALY_DETECTION_IN_IOT_DEVICES}

\bibitem{ref3}
Y. R. Siwakoti, D. B. Rawat, and S.-H. Loke, ``AD-GAM: Anomaly Detection in IoT Using Generative AI Models,'' \textit{Proceedings of SPIE}, vol. 13473, 2025. [Online]. Available: \url{https://www.spiedigitallibrary.org/conference-proceedings-of-spie/13473/134730J/AD-GAM--anomaly-detection-in-IoT-using-generative-AI/10.1117/12.3058632.full}

\bibitem{ref4}
C. Smith, F. Johnson, and G. Williams, ``Review: Generative Adversarial Networks-Enabled Anomaly Detection in IoT,'' \textit{Expert Systems with Applications}, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S0957417425025953}

\bibitem{ref5}
D. Brown, H. Davis, and I. Miller, ``AI-Driven Anomaly Detection for Securing IoT Devices in Smart Cities,'' \textit{Electronics}, vol. 14, no. 12, p. 2492, 2025. [Online]. Available: \url{https://www.mdpi.com/2079-9292/14/12/2492}

\bibitem{ref6}
E. Wilson, J. Moore, and K. Taylor, ``Privacy-Preserving Security of IoT Networks: A Comparative Study,'' \textit{ScienceDirect}, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2772918425000013}

\bibitem{ref7}
I. Ficili, M. Giacobbe, G. Tricomi, and A. Puliafito, ``From Sensors to Data Intelligence: Leveraging IoT, Cloud, and Edge Computing with AI,'' \textit{Sensors}, vol. 25, no. 6, p. 1763, 2025. [Online]. Available: \url{https://www.mdpi.com/1424-8220/25/6/1763}

\bibitem{ref8}
H. Yar, A. S. Imran, Z. A. Khan, M. Sajjad, and Z. Kastrati, ``Towards Smart Home Automation Using IoT-Enabled Edge-Computing Paradigm,'' \textit{Sensors}, vol. 21, no. 14, p. 4932, 2021. [Online]. Available: \url{https://www.researchgate.net/publication/353372471_Towards_Smart_Home_Automation_Using_IoT-Enabled_Edge-Computing_Paradigm}

\bibitem{ref9}
Z. Y. M. Yusoff, M. K. Ishak, L. A. B. Rahim, and M. S. M. Asaari, ``Improving Smart Home Security via MQTT: Maximizing Data Privacy and Device Authentication Using Elliptic Curve Cryptography,'' \textit{Computer Systems Science and Engineering}, vol. 48, no. 6, pp. 1669--1697, 2024. [Online]. Available: \url{https://www.techscience.com/csse/v48n6/58697}

\bibitem{ref10}
R. A. Mahajan, R. G. Mahajan, M. Tatiya, U. H. Mandekar, M. Shahakar, and Y. Patil, ``Enhancing MQTT Security in the Internet of Things with an Enhanced Symmetric Algorithm,'' \textit{Journal of Electrical Systems}, vol. 20, no. 1s, pp. 126--137, 2024. [Online]. Available: \url{https://www.researchgate.net/publication/379866006_Enhancing_MQTT_Security_in_the_Internet_of_Things_with_an_Enhanced_Symmetric_Algorithm}

\bibitem{ref11}
L. Micheal, ``Multi-Modal Generative AI for Predictive Maintenance: Integrating Vibration, Acoustic, and Visual Data for Holistic Failure Forecasting,'' \textit{ResearchGate}, 2024. [Online]. Available: \url{https://www.researchgate.net/publication/395791187_Hybrid_Approaches_to_Predictive_Maintenance_Combining_Generative_AI_with_IoT_Sensor_Data_for_Enhanced_Failure_Prediction}

\bibitem{ref12}
Y. L. Aung, I. Christian, Y. Dong, X. Ye, S. Chattopadhyay, and J. Zhou, ``Generative AI for Internet of Things Security: Challenges and Opportunities,'' \textit{arXiv preprint arXiv:2502.08886}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2502.08886}

\bibitem{ref13}
V. L. Petrovic, ``The Role of Smart Homes in Providing Care for Older Adults: A Systematic Literature Review from 2010 to 2023,'' \textit{Smart Cities}, vol. 7, no. 4, pp. 1653--1684, 2024. [Online]. Available: \url{https://www.mdpi.com/2624-6511/7/4/62}

\bibitem{ref14}
M. S. Hassan, Q. Ali, and R. Khan, ``Internet of Robotic Things for Independent Living Support,'' \textit{Procedia Computer Science}, vol. 235, pp. 45--54, 2024. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2542660524000623}

\bibitem{ref15}
N. Zhang, S. Li, and T. Wang, ``Design and Implementation of Smart Home System Based on IoT,'' \textit{Measurement: Sensors}, vol. 32, p. 101056, 2024. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2590123024016621}

\bibitem{ref16}
O. Gomez, U. Martinez, and V. Sanchez, ``Design of an Innovative Solution to Integrate and Automate Smart Homes via Multiple Chatbots,'' \textit{Procedia Computer Science}, vol. 248, pp. 112--121, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2542660525002070}

\bibitem{ref17}
P. Kumar, W. Singh, and X. Chen, ``Adoption of Internet of Things in Residential Smart Homes: A User-Centric Study,'' \textit{Digital Business}, vol. 5, no. 1, p. 100092, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2666188825002333}

\bibitem{ref18}
Q. Mahmoud, Y. Zhang, and Z. Ali, ``MQTT\_UAD: MQTT Under Attack Dataset — A Public Benchmark for MQTT Security Research,'' \textit{Data in Brief}, vol. 58, p. 111245, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2352340925008881}

\bibitem{ref19}
R. Gupta, A. Kumar, and B. Sharma, ``Edge AI Enabled IoT Framework for Secure Smart Home Infrastructure,'' \textit{Procedia Computer Science}, vol. 230, pp. 88--97, 2024. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S1877050924009979}

\bibitem{ref20}
S. Patel, C. Reddy, and D. Gupta, ``Machine Learning and IoT in Healthcare: Challenges and Future Directions,'' \textit{Informatics in Medicine Unlocked}, vol. 45, p. 101432, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2543106425000201}

\bibitem{ref21}
M. M. Shafi and A. H. Lashkari, ``Toward generating a large-scale IoT-Zwave intrusion detection dataset: Smart device profiling, intruders behavior, and traffic characterization,'' \textit{Internet of Things}, vol. 34, p. 101747, 2025. [Online]. Available: \url{https://www.sciencedirect.com/science/article/pii/S2542660525002616}

\bibitem{ref22}
A. Chakraborty, M. Islam, F. Shahriyar, S. Islam, H. U. Zaman, and M. Hasan, ``Smart Home System: A Comprehensive Review,'' \textit{Journal of Sensors}, vol. 2023, p. 7616683, 2023. [Online]. Available: \url{https://onlinelibrary.wiley.com/doi/epdf/10.1155/2023/7616683}

\bibitem{ref23}
R. Wolniak and K. Stecuła, ``Artificial Intelligence in Smart Cities—Applications, Barriers, and Future Directions: A Review,'' \textit{Smart Cities}, vol. 7, no. 3, pp. 1346--1389, 2024. [Online]. Available: \url{https://www.mdpi.com/2624-6511/7/3/57}

\bibitem{ref24}
M. Poyyamozhi, B. Murugesan, N. Rajamanickam, M. Shorfuzzaman, and Y. Aboelmagd, ``IoT—A Promising Solution to Energy Management in Smart Buildings: A Systematic Review, Applications, Barriers, and Future Scope,'' \textit{Buildings}, vol. 14, no. 11, p. 3446, 2024. [Online]. Available: \url{https://www.mdpi.com/2075-5309/14/11/3446}

\bibitem{ref25}
G. Vardakis, G. Hatzivasilis, E. Koutsaki, and N. Papadakis, ``Review of Smart-Home Security Using the Internet of Things,'' \textit{Electronics}, vol. 13, no. 16, p. 3343, 2024. [Online]. Available: \url{https://www.mdpi.com/2079-9292/13/16/3343}

\bibitem{ref26}
Y. Park and J. Han, ``Smart Home Advancements for Health Care and Beyond: Systematic Review of Two Decades of User-Centric Innovation,'' \textit{Journal of Medical Internet Research}, vol. 27, p. e62793, 2025. [Online]. Available: \url{https://www.jmir.org/2025/1/e62793}

\bibitem{ref27}
F. El Husseini, H. N. Noura, O. Salman, and K. Chahine, ``Machine Learning in Smart Buildings: A Review of Methods, Challenges, and Future Trends,'' \textit{Applied Sciences}, vol. 15, no. 14, p. 7682, 2025. [Online]. Available: \url{https://www.mdpi.com/2076-3417/15/14/7682}

\bibitem{ref28}
A. Choudhary, ``Internet of Things: a comprehensive overview, architectures, applications, simulation tools, challenges and future directions,'' \textit{Discover Internet of Things}, vol. 4, no. 1, p. 31, 2024. [Online]. Available: \url{https://link.springer.com/article/10.1007/s43926-024-00084-3}

\bibitem{ref29}
J. Rajasekhar, T. Thanusha, G. N. Jyothi, K. Tejaswi, and L. Abualigah, ``IoT Based Security and Privacy Implementation in Smart Home,'' \textit{International Journal of Food and Nutritional Sciences}, vol. 11, no. 5, pp. 2351--2360, 2022. [Online]. Available: \url{https://www.ijfans.org/issue?volume=Volume%2011&issue=Issue%205&year=2022}

\bibitem{ref30}
A. Winarno, Widodo, and M. K. Nurcahyo, ``Smart Home Remote Control System Prototype Using Internet of Things (IoT) Based ESP8266 Microcontroller,'' \textit{BEST: Journal of Applied Electrical \& Science Technology}, vol. 5, no. 2, pp. 29--36, 2023. [Online]. Available: \url{https://jurnal.unipasby.ac.id/best/article/view/8063}

\bibitem{ref31}
S. M. Geddam, A. B. Gowda, and A. Hussain, ``IoT Enabled Smart Homes in Tropical Regions as a Means of Sustainable Development,'' \textit{IOP Conference Series: Earth and Environmental Science}, vol. 1306, p. 012035, 2024. [Online]. Available: \url{https://iopscience.iop.org/article/10.1088/1755-1315/1306/1/012035}

\bibitem{ref32}
J. L. L. Delgado and J. A. L. Ramos, ``A Comprehensive Survey on Generative AI Solutions in IoT Security,'' \textit{Electronics}, vol. 13, no. 24, p. 4965, 2024. [Online]. Available: \url{https://www.mdpi.com/2079-9292/13/24/4965}

\bibitem{ref33}
R. Priyakanth, N. M. S. Krishna, G. Karanam, M. L. Prassna, S. B. Poojitha, and J. Mounika, ``IoT Based Smart Door Unlock and Intruder Alert System,'' \textit{ResearchGate}, 2021. [Online]. Available: \url{https://www.researchgate.net/publication/356206104_IoT_Based_Smart_Door_Unlock_and_Intruder_Alert_System}

\bibitem{ref34}
P. Jaya, R. Fikri, A. D. Samala, and D. Sanjaya, ``An internet of things-driven smart key system with real-time alerts: innovations in hotel security,'' \textit{International Journal of Reconfigurable and Embedded Systems}, vol. 14, no. 1, pp. 145--156, 2025. [Online]. Available: \url{https://ijres.iaescore.com/index.php/IJRES/article/view/21213}

\bibitem{ref35}
P. S. B. Macheso, T. D. Manda, S. W. Chisale, N. Dzupire, J. S. P. Mlatho, and D. Mukanyiligira, ``Design of ESP8266 Smart Home Using MQTT and Node-RED,'' \textit{International Conference on Artificial Intelligence and Smart Systems (ICAIS)}, pp. 1004--1009, 2021. [Online]. Available: \url{https://www.researchgate.net/publication/350935668_Design_of_ESP8266_Smart_Home_Using_MQTT_and_Node-RED}

\bibitem{ref36}
A. Ahmed, V. Ungureanu, T. Gaber, C. Watterson, and F. Masmoudi, ``Smart Home Privacy: A Scoping Review,'' \textit{Proceedings of the 10th International Conference on Information Systems Security and Privacy (ICISSP)}, pp. 635--642, 2024. [Online]. Available: \url{https://www.scitepress.org/Papers/2024/122559/122559.pdf}

\bibitem{ref37}
L. Abualigah et al., ``IoT based security and privacy implementation in smart home,'' \textit{ResearchGate}, 2024. [Online]. Available: \url{https://www.researchgate.net/publication/378739063_IoT_based_security_and_privacy_implementation_in_smart_home}

\bibitem{ref38}
Y. L. Aung, I. Christian, Y. Dong, X. Ye, S. Chattopadhyay, and J. Zhou, ``Generative AI for Internet of Things Security: Challenges and Opportunities,'' \textit{arXiv preprint arXiv:2502.08886}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2502.08886}

\bibitem{ref39}
Y. R. Siwakoti, D. B. Rawat, and S.-H. Loke, ``AD-GAM: Anomaly Detection in IoT Using Generative AI Models,'' \textit{Proceedings of SPIE}, vol. 13473, p. 134730J, 2025. [Online]. Available: \url{https://www.spiedigitallibrary.org/conference-proceedings-of-spie/13473/134730J/AD-GAM--anomaly-detection-in-IoT-using-generative-AI/10.1117/12.3058632.full}

\bibitem{ref40}
P. Cosper, ``Generative AI Techniques for Anomaly Detection in IoT Devices,'' \textit{ResearchGate}, 2025. [Online]. Available: \url{https://www.researchgate.net/publication/395801862_GENERATIVE_AI_TECHNIQUES_FOR_ANOMALY_DETECTION_IN_IOT_DEVICES}

\bibitem{ref41}
M. A. I. Mahmood, F. Ashab, M. S. Sohan, M. H. I. Chy, and M. F. Kader, ``LLM-Enhanced Security Framework for IoT Network: Anomaly Detection and Malicious Devices Identification,'' \textit{IEEE Access}, vol. 13, pp. 12450--12465, 2025. [Online]. Available: \url{https://ieeexplore.ieee.org/document/11175688}

\bibitem{ref42}
D. A. Worae and S. Mastorakis, ``An LLM-Powered AI Agent Framework for Holistic IoT Traffic Interpretation,'' \textit{arXiv preprint arXiv:2510.13925}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2510.13925}

\bibitem{ref43}
A. G. Ayad, M. M. El-Gayar, N. A. Hikal, and N. A. Sakr, ``Efficient Real-Time Anomaly Detection in IoT Networks Using One-Class Autoencoder and Deep Neural Network,'' \textit{Electronics}, vol. 14, no. 1, p. 104, 2025. [Online]. Available: \url{https://www.mdpi.com/2079-9292/14/1/104}

\bibitem{ref44}
K. Khatiwada, J. Hopper, J. Cheatham, A. Joshi, and S. Baidya, ``Large Language Models in the IoT Ecosystem: A Survey on Security Challenges and Applications,'' \textit{arXiv preprint arXiv:2505.17586}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2505.17586}

\bibitem{ref45}
N. S. Alotaibi, H. I. S. Ahmed, S. O. M. Kamel, and G. F. ElKabbany, ``Secure Enhancement for MQTT Protocol Using Distributed Machine Learning Framework,'' \textit{Sensors}, vol. 24, no. 5, p. 1638, 2024. [Online]. Available: \url{https://www.mdpi.com/1424-8220/24/5/1638}

\bibitem{ref46}
E. Dritsas and M. Trigka, ``Federated Learning for IoT: A Survey of Techniques, Challenges, and Applications,'' \textit{Journal of Sensor and Actuator Networks}, vol. 14, no. 1, p. 9, 2025. [Online]. Available: \url{https://www.mdpi.com/2224-2708/14/1/9}

\bibitem{ref47}
R. Vidhya, D. Loganathan, S. Saranya, P. N. Periyasamy, and S. Sumathi, ``Anomaly Detection in IoT Networks Using Federated Machine Learning Approaches,'' \textit{International Journal of Computational and Experimental Science and Engineering}, vol. 11, no. 3, pp. 4399--4407, 2025. [Online]. Available: \url{https://www.researchgate.net/publication/392893320_Anomaly_Detection_in_IoT_Networks_Using_Federated_Machine_Learning_Approaches}

\bibitem{ref48}
F. Asiri, W. A. Malwi, F. Masood, M. S. Alshehri, T. Zhukabayeva, S. A. Shah, and J. Ahmad, ``Privacy Preserving Federated Anomaly Detection in IoT Edge Computing Using Bayesian Game Reinforcement Learning,'' \textit{Computers, Materials \& Continua}, vol. 84, no. 2, pp. 3943--3960, 2025. [Online]. Available: \url{https://www.techscience.com/cmc/v84n2/62939}

\bibitem{ref49}
S. Kaganurmath, N. G. Cholli, and M. R. Anala, ``DLKS-MQTT: A Lightweight Key Sharing Protocol for Secure IoT Communications,'' \textit{Engineering, Technology \& Applied Science Research}, vol. 15, no. 2, pp. 21532--21538, 2025. [Online]. Available: \url{https://etasr.com/index.php/ETASR/article/view/10216}

\bibitem{ref50}
M. Salama, ``Integrating blockchain, MQTT, and machine learning for enhanced IoT applications: A comprehensive survey,'' \textit{Sakarya University Journal of Computer and Information Sciences}, vol. 8, no. 3, pp. 570--591, 2025. [Online]. Available: \url{https://www.researchgate.net/publication/396031010_Integrating_Blockchain_MQTT_and_Machine_Learning_for_Enhanced_IoT_Applications_A_Comprehensive_Survey}

\bibitem{ref51}
M. Salama and B. Raslen, ``MQTT in Action: Building Reliable and Scalable Home Automation Systems,'' \textit{Sakarya University Journal of Computer and Information Sciences}, vol. 7, no. 3, pp. 494--509, 2024. [Online]. Available: \url{https://www.researchgate.net/publication/387574531_MQTT_in_Action_Building_Reliable_and_Scalable_Home_Automation_Systems}

\bibitem{ref52}
A. Gono, I. Pisařovic, M. Zejda, J. Landa, and D. Procházka, ``Improving IoT Management with Blockchain: Smart Home Access Control,'' \textit{EJOBSAT}, vol. 10, no. 2, pp. 225--240, 2024. [Online]. Available: \url{https://www.ejobsat.cz/pdfs/ejo/2024/02/04.pdf}

\bibitem{ref53}
J. Liu et al., ``A Survey on Diffusion Models for Anomaly Detection,'' \textit{arXiv preprint arXiv:2501.11430}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2501.11430}

\bibitem{ref54}
J. Zhou, X. Yang, and Z. Ren, ``SCConv-Denoising Diffusion Probabilistic Model Anomaly Detection Based on TimesNet,'' \textit{Electronics}, vol. 14, no. 4, p. 746, 2025. [Online]. Available: \url{https://www.mdpi.com/2079-9292/14/4/746}

\bibitem{ref55}
V. Livernoche, V. Jain, Y. Hezaveh, and S. Ravanbakhsh, ``On Diffusion Modeling for Anomaly Detection,'' \textit{International Conference on Learning Representations (ICLR)}, 2024. [Online]. Available: \url{https://openreview.net/forum?id=lR3rk7ysXz}

\bibitem{ref56}
Y. Liu et al., ``Anomaly Detection and Generation with Diffusion Models: A Survey,'' \textit{arXiv preprint arXiv:2506.09368}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2506.09368}

\bibitem{ref57}
M. A. Addow, A. D. Jimale, A. Y. Nageye, M. O. Abdullahi, and S. M. Hilowle, ``A low-cost IoT-based smart home automation system for urban sustainability in Mogadishu, Somalia,'' \textit{Discover Internet of Things}, vol. 5, no. 1, p. 99, 2025. [Online]. Available: \url{https://link.springer.com/article/10.1007/s43926-025-00207-4}

\bibitem{ref58}
L. Thomas et al., ``Generative AI advances for data-driven insights in IoT cloud technologies and big data challenges,'' \textit{ResearchGate}, 2025. [Online]. Available: \url{https://www.researchgate.net/publication/387773122_Generative_AI_advances_for_data-driven_insights_in_IoT_cloud_technologies_and_big_data_challenges}

\bibitem{ref59}
D. Rupanetti and N. Kaabouch, ``Combining Edge Computing-Assisted Internet of Things Security with Artificial Intelligence: Applications, Challenges, and Opportunities,'' \textit{Applied Sciences}, vol. 14, no. 16, p. 7104, 2024. [Online]. Available: \url{https://www.mdpi.com/2076-3417/14/16/7104}

\bibitem{ref60}
M. A. Husnoo, A. Anwar, R. K. Chakrabortty, R. Doss, and M. J. Ryan, ``Differential Privacy for IoT-Enabled Critical Infrastructure: A Comprehensive Survey,'' \textit{IEEE Access}, vol. 9, pp. 145500--145525, 2021. [Online]. Available: \url{https://www.researchgate.net/publication/355765966_Differential_Privacy_for_IoT-Enabled_Critical_Infrastructure_A_Comprehensive_Survey}

\bibitem{ref61}
M. Shalan, M. R. Hasan, Y. Bai, and G. Hosanna, ``Enhancing Smart Home Security: Blockchain-Enabled Federated Learning with Knowledge Distillation for Intrusion Detection,'' \textit{Smart Cities}, vol. 8, no. 1, pp. 120--145, 2025. [Online]. Available: \url{https://www.mdpi.com/2624-6511/8/1/35}
...
% ==========================================
\chapter{Hardware Schematics and Pin Configuration}

\noindent This appendix details the physical wiring and hardware interface of the IoTShield edge node. The ESP32-WROOM-32 microcontroller was selected as the central processing unit due to its dual-core architecture, which is essential for isolating cryptographic operations from continuous sensor polling.

\vspace{0.3cm}

\section*{A.1 Sensor Pin Mappings}
\noindent Table A.1 defines the specific General Purpose Input/Output (GPIO) pins utilized to interface the environmental sensors with the ESP32.

\vspace{0.5cm}

\begin{table}[H]
    \centering
    \caption{ESP32 GPIO Pin Configuration for Sensor Integration}
    \vspace{0.2cm}
    \renewcommand{\arraystretch}{1.5}
    \begin{tabular}{|l|l|l|}
        \hline
        \textbf{Component} & \textbf{ESP32 Pin} & \textbf{Signal Type} \\
        \hline
        ...
        \hline
    \end{tabular}
    \label{tab:appendix_pin_mapping}
\end{table}

\vspace{0.5cm}

% ==========================================
% APPENDIX B: Firmware (ESP32)
% ==========================================
\chapter{Core Firmware Implementation (ESP32)}

\noindent This appendix provides essential excerpts from the C++ firmware deployed on the ESP32 edge node. The code highlights the implementation of the Differential Privacy (DP) layer and the hardware-accelerated RSA-2048 encryption logic.

\vspace{0.3cm}

\section*{B.1 Differential Privacy: Gaussian Noise Injection}
\noindent The following function demonstrates how calibrated noise ($\epsilon = 0.5$) is dynamically generated and applied to the raw analog readings to prevent behavioral profiling.

\vspace{0.3cm}

\begin{lstlisting}[language=C++, showspaces=false, showstringspaces=false, basicstyle=\ttfamily\small, caption={Gaussian noise injection logic on the ESP32}, label={lst:gaussian_noise}]
#include <math.h>
...
% ==========================================
% APPENDIX C: Backend AI (Django)
% ==========================================
\chapter{Backend AI Integration (Django \& Llama 3.2)}

\noindent To ensure deterministic and highly relevant safety diagnostics, the Llama 3.2 (1B) model required a strictly engineered system prompt. This appendix details the core Python logic used within the Django backend (via the \texttt{OllamaAnomalyDetector} class) to dynamically format telemetry, inject contextual baselines, and query the local Ollama REST API.

\vspace{0.3cm}

\section*{C.1 Dynamic LLM Prompt Engineering}
...
\begin{lstlisting}[language=Python, showspaces=false, showstringspaces=false, basicstyle=\ttfamily\footnotesize, caption={Excerpt from ollama\_anomaly\_detector.py detailing the dynamic prompt construction and API payload}, label={lst:llm_prompt}]
import requests
import json
